...
| `review-whisper` | Surface files flagged for human verification (any `whisper_review_needed`, regardless of source) |
...
These exist because we already hit the same bugs in batch runs. Use them instead of writing one-off scripts.

| Script | What it does | When to call |
|---|---|---|
| [`scripts/apply_enrichment.py`](./scripts/apply_enrichment.py) | The **only** supported write path for enrichment. Validates, auto-trims oversize summaries, snaps `key_claims`/`detailed_notes` timestamps to nearest preceding marker, dedupes/caps topics, mirrors `tags_topic`, sets enrichment metadata, inserts body sections. Takes a JSON payload via `--json` or `--stdin`. Supports `--dry-run`. | Every per-file enrichment write |
| [`scripts/post_enrich.py`](./scripts/post_enrich.py) | Wraps `validate_corpus → build_index → corpus_status`. If validate fails, the index is **not** rebuilt. | After every enrichment batch (mandatory) |
| [`scripts/list_whisper.py`](./scripts/list_whisper.py) | Ranks files carrying `whisper_review_needed` by view count. The walkthrough itself is agent-driven per [`operations/review-whisper/INSTRUCTIONS.md`](./operations/review-whisper/INSTRUCTIONS.md). | When surfacing review backlog |
