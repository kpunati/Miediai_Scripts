...
5. **Merge** the enrichment into the existing frontmatter:
   - Update each enrichment field with the generated value.
   - Set `enriched: true`.
   - Set `enrichment_date` to today (UTC, `YYYY-MM-DD`).
   - Set `enrichment_version: 1` (bump only if instructions in this file change materially).
   - Mirror `topics` into `tags_topic` (legacy alias — keep them identical).
6. **Insert body sections** before the existing `## Transcript` section. Enrichment adds three required readable sections so the file is genuinely useful when someone opens it. See "Body sections" below for content rules.
...
8. **Write** the file back. Preserve the title heading, header block (Channel/Published/URL/Duration), and the original `## Transcript` section byte-for-byte. The frontmatter is rewritten with enrichment fields populated; the new body sections are inserted between the header block and `## Transcript`.
9. **Log** to the chat: filename, summary (one line), topic count, entity count, time taken.

After the batch: tell the user to run `rebuild-index` to refresh `output/index.csv`.
...
- Suggest running `rebuild-index` (and `validate` if any file failed).
